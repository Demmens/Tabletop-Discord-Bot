	...
	async exec(message, args) {
		...
		//Removechannel command
			...
			for (let ch in message.guild.channels.cache){
				message.channel.send("Name: " + ch[1].name+' Category: '+ch[1].parent.name)
				if (chName == ch[1].name && ch[1].type != "category" && isGM(ch[1].parent.name)){ //Search through channels in games they GM.
